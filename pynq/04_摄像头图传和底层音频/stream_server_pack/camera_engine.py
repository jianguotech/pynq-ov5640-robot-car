...
class CameraEngine:
    ...
    def __init__(self, vdma_base=0x43000000,
        ...
        # Some bitstreams keep REG_PARK at 0 even while S2MM buffers update.
        # In that case, periodically sample the ring buffers instead of
        # waiting forever for a park-pointer transition.
        self.park_fallback_interval = 1.0 / 30.0
    ...
    def _capture_loop(self):
        ...
        while self._running:
            ...
            park_changed = (park != last_park)
            if park_changed:
                last_park = park
                safe_idx = (park - 1) % len(self.frames)
            else:
                if time.time() - last_emit < self.park_fallback_interval:
                    time.sleep(0.001)
                    continue
                safe_idx = fallback_idx
                fallback_idx = (fallback_idx + 1) % len(self.frames)
                with self._stats_lock:
                    self.stats['park_fallbacks'] += 1

            # Paranoid re-check: skip if VDMA is now writing this buffer.
            # When REG_PARK is stuck, fallback frames must bypass this check.
            if park_changed and (self.vdma.read(self.REG_PARK) & 0x3) == safe_idx:
                with self._stats_lock:
                    self.stats['drops'] += 1
                continue
            ...
            elapsed_ms = (publish_time - t_start) * 1000
